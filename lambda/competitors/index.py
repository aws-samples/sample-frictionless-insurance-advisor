"""
Competitor products Lambda — returns reference information about competitor
products concatenated from S3 markdown files
"""
import json
import boto3
import os
...
def get_competitors_data():
    """Read all files from competitors S3 bucket and concatenate content"""
    bucket_name = os.environ.get('COMPETITORS_BUCKET')
    if not bucket_name:
        raise ValueError("COMPETITORS_BUCKET environment variable not set")

    try:
        response = s3_client.list_objects_v2(Bucket=bucket_name)

        if 'Contents' not in response:
            return "No competitors data available"
...
def handler(event, context):
    """Main Lambda handler for competitors operations"""
    try:
        ...
        competitors_content = get_competitors_data()

        result = {
            'competitors_data': competitors_content
        }

        return create_response(200, result)

    except Exception as e:
        print(f"Error in competitors handler: {str(e)}")
