"""
Competitive differentiation Lambda — returns Unicorn Insurance competitive
talking points concatenated from S3 markdown files
"""
import json
import boto3
import os
...
def get_competitive_data():
    """Read all files from competitive S3 bucket and concatenate content"""
    bucket_name = os.environ.get('COMPETITIVE_BUCKET')
    if not bucket_name:
        raise ValueError("COMPETITIVE_BUCKET environment variable not set")
...
def handler(event, context):
    """Main Lambda handler for competitive operations"""
    try:
        ...
        competitive_content = get_competitive_data()

        result = {
            'competitive_data': competitive_content
        }

        return create_response(200, result)

    except Exception as e:
        print(f"Error in competitive handler: {str(e)}")
